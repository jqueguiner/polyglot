...
from gpt_2_simple.src.monitoring import Monitoring
...
def finetune(sess,
    ...
    assert model_name not in ['774M', '1558M'] or multi_gpu, "[ERROR] Currently, a modern single GPU cannot finetune the 774M GPT-2 model or larger."
    ...
    for file in ['hparams.json', 'encoder.json', 'vocab.bpe']:
        try:
            ...
            print("[ERROR] You need to download the GPT-2 model first via download_gpt2()")
            raise(fnf_error)
    ...
    if sample_length > hparams.n_ctx:
        raise ValueError(
            "[ERROR] Can't get samples longer than window size: %s" % hparams.n_ctx)
    ...
    if accumulate_gradients > 1:
        if use_memory_saving_gradients:
            exit("[INFO] Memory saving gradients are not implemented for gradient accumulation yet.")
    ...
    try:
        while True:
            ...
            if (counter - 1) % save_every == 0 and counter > 1:
                ...
                monitoring.push_text('generated_text', sample_text, counter, v_loss, t)


            if v_loss < best_loss and v_loss > 1.00:
                best_loss = v_loss
                loss_text = generate_samples(False)
                loss_sample_text = "===== LOSS : " + str(best_loss) + "=====" + loss_text
                ...
                monitoring.push_text('best_loss_text', loss_text, counter, best_loss, t)
            ...
            if counter % print_every == 0:
                ...
                monitoring.push_metric('avg_loss', avg_loss[0]/avg_loss[1], counter, t)
                print(
...
def load_gpt2(sess,
    ...
    if model_name:
        print('[INFO] Loading pretrained model', ckpt)
    else:
        print('[INFO] Loading checkpoint', ckpt)
    saver.restore(sess, ckpt)
...
def cmd():
    ...
    parser.add_argument(
        ...
        nargs='?', default=True, type=lambda x: (str(x).lower() == 'true'))
    parser.add_argument(
        '--reporting_config',  help="[finetune] path to monitoring config file default : ~/.gpt_2_simple/config.yml",
        nargs='?', default="%s/.gpt_2_simple/config.yml" % str(Path.home()), type=str)
...
def cmd_finetune(dataset, run_name, checkpoint_dir, model_name, model_dir, steps,
                 ...
                 save_every, print_every, optimizer, overwrite, multi_gpu, reporting_config):
    ...
    global monitoring
    monitoring = Monitoring(reporting_config, run_name)
